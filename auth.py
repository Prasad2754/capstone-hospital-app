...
def register():
    try:
        ...
        # If phone is provided, check if it exists in the database
        if phone:
        ...
        # Insert user into the database depending on whether phone is provided or not
        if phone:
...
def login():
    try:
        # Getting data from the request (JSON)
        data = request.get_json()
        login_id = data.get("login_id")  # Could be either email or phone
        password = data.get("password")
        ...
        if user and verify_password(password, user[3]):
            ...
            # Check if there was a pending doctor appointment to book
